...
from .misc import check_file_exists, get_output_filename
...
def decay_correction(time, start_time, half_life):
    ...
    return np.exp(
        decay_constant * 60 * (time - start_time)
...
def main():
    parser = argparse.ArgumentParser(
        description="Sum PET NIFTI images with decay correction."
    )
    ...
    parser.add_argument(
        "--half-life", type=float, required=True, help="Half-life in seconds"
    )
    parser.add_argument(
        "--start-times",
        type=list,
        required=True,
        help="Comma-separated list of start times (in minutes)",
    )
    parser.add_argument(
        "--durations",
        type=list,
        required=True,
        help="Comma-separated list of durations (in minutes)",
    )
    parser.add_argument(
        "--decay-corrections",
        type=list,
        required=True,
        help="Comma-separated list of decay corrections",
    )
    parser.add_argument("--scale", type=float, default=1.0, help="Final scale factor")
    parser.add_argument(
        "--use-first-decay",
        action="store_true",
        help="Use decay correction of the first frame",
    )
    parser.add_argument(
        "--output_dir", type=str, default=None, required=False, help="Output directory"
    )
