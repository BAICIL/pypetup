...
def run_pup(pet_nifti, fs_dir, pet_json=None, derivatives_dir=None, t1_filename='orig_nu.mgz', start_time=None, duration=None, norsf=False):
    ...
    if pet_json is None:
            pet_json = os.path.join(input_pet_dir, input_pet_filename_without_extension + '.json')
    ...
    copy_pet_json = os.path.join(process_folder, input_pet_filename_without_extension + '.json')
    ...
    msumfile = pup.time_function(pup.model_sum_pet, *(coregfile, start_frame, end_frame, half_life, start_times, durations, decay_factors))
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Process PET data using the PUP workflow.")
    parser.add_argument('--pet_nifti', type=str, required=True, help="Path to the input PET file.")
    parser.add_argument('--pet_json', type=str, required=False, default=None, help="Path to the input PET JSON file.")
    parser.add_argument('--derivatives_dir', type=str, required=False, default=None, help="Path to the base directory (optonal, default=None).")
    parser.add_argument('--fs_dir', type=str, required=True, help="Path of the Subjects FreeSurfer mri directory.")
    parser.add_argument('--t1_filename', type=str, required=False, default='orig_nu.mgz', help="File name with extension of the T1 file (optional, default=orig_nu.mgz)")
    parser.add_argument('--start_time', type=float, required=False, default=None, help="Start time for frames of interest (optional, default=None)")
    parser.add_argument('--duration', type=float, required=False, default=None, help="Total duration of frames of interest (optional, default=None)")
    parser.add_argument('--norsf', action='store_true', help="Do not perform RSF correction")

    args = parser.parse_args()

    result = pup.time_function(run_pup, *(args.pet_nifti, args.fs_dir, args.pet_json, args.devivatives_dir, args.t1_filename, args.start_time, args.duration, args.norsf))
