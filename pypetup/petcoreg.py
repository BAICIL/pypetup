...
from .misc import check_file_exists, check_fsl_installation, get_output_filename
...
def main():
    parser = argparse.ArgumentParser(
        description="Coregister PET image to T1 image using FSL"
    )
    parser.add_argument(
        "--pet_file",
        type=str,
        help="Input PET image file (nii.gz format)",
        required=True,
    )
    parser.add_argument(
        "--t1_file", type=str, help="Input T1 image file (nii.gz format)", required=True
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Path to the output directory (optional).",
        required=False,
    )
