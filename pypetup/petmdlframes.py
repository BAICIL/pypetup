...
def find_frame_indices(json_file, start_time: float = None, duration: float = None):
    ...
    if len(start_times) == 1:
        if start_time == start_times[0] and duration == total_duration:
            ...
            raise ValueError("The specified time range does not fit within the single frame.")
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Compute the start and end frame indices based on input time and duration.')
    
    parser.add_argument('--json_file', type=str, help='Path to the JSON file containing frame times.', required=True)
    parser.add_argument('--start_time', type=float, default=None, help='Start time in minutes.', required=False)
    parser.add_argument('--duration', type=float, default=None, help='Duration in minutes.', required=False)
    
    args = parser.parse_args()
    
    try:
        # Calculate start and end frame
        start_frame, end_frame = find_frame_indices(args.json_file, args.start_time, args.duration)
